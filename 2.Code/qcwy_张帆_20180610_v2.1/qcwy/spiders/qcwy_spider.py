...
class TestfollowSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        if os.path.exists('./sites/51job'):
            print('自定义start_url开始执行')
            #读取网址，分析网址，去除需要的元素,需om要将网址复制到当前目录下的./weblist.txt中
            path = './sites/51job/'
            files = os.listdir(path)
            for file in files:
                filename = os.path.join('%s%s' %(path,file))
            ...
            print('当前路径下不存在51job路径，必须手动添加才可以进行爬取操作！')
    ...
    def parse(self,response):
        ...
        for result in results:
            ...
            tmp_url = result.xpath('@href').extract()[0] #链接,不需要转换成utf8编码，转换成Unicode编码（因为服务器端使用的才是Unicode，保存文件时使用的是utf8编码）
    ...
    def parse_range(self, response):
        ...
        for site in sites:
            ...
            # print(type(a))
            '''
            之前显示的是unicode字符，将其编码成utf8字符进行显示
            '''
            ...
            #获取数据库的主键
            item['id'] = item['parent_link'] + '/' + str(random.randint(0,10000))

            item['city'] = response.meta['city']

            # 将获取的链接封装成request请求，传给schedule
            #对应的URL必须是Unicode编码，要将item['link']解码成Unicode
            request = Request(item['link'].decode('utf-8'), callback=self.parse_contents)
